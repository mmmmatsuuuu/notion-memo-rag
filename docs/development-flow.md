...
1. `/` に検索UIを作成する（ログイン後のみ表示）。
   - 画面下部固定ツールバーに長文入力 textarea
   - 同ツールバーに検索実行ボタン（topK=6）
   - 同ツールバーに `/admin` 導線ボタン
   - 結果カード（書誌情報、400文字プレビュー、一次資料導線）
   - 一次資料導線は各カード内に表示
2. `/admin` の最小画面を作成する。
   - `Sync latest 50`
   - `Sync all`
...
## 4. 検索機能（`/api/search`）
1. 認証確認と `ALLOWED_EMAIL` の検証を実装する。
2. 入力文脈から embedding を生成する。
3. `match_memos` を実行し、`topK=6` 固定で取得する。
4. 結果をカード表示し、一次資料導線（Scholar クエリ）を提示する。
